...
class ModuleExTest < Test::Unit::TestCase
  ...
  def test_should_use_regular_objects_as_drops
    assert_equal 'allowedA', Liquid2::Template.parse("{{ a.allowedA }}").render('a'=>@a)
    assert_equal 'allowedB', Liquid2::Template.parse("{{ a.chainedB.allowedB }}").render('a'=>@a)
    assert_equal 'allowedC', Liquid2::Template.parse("{{ a.chainedB.chainedC.allowedC }}").render('a'=>@a)
    assert_equal 'another_allowedC', Liquid2::Template.parse("{{ a.chainedB.chainedC.another_allowedC }}").render('a'=>@a)
    assert_equal '', Liquid2::Template.parse("{{ a.restricted }}").render('a'=>@a)
    assert_equal '', Liquid2::Template.parse("{{ a.unknown }}").render('a'=>@a)
  end
end # ModuleExTest
